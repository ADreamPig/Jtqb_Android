...
public class LockPattern extends View {
    ...
    private int innerColor = 0XFFFFFFFF;// 内圆颜色
    private int outerColor = 0XFFFFFFFF;// 外圆颜色

    private int selectedInnerColor = 0XFFFFFFFF;// 选中时内圆颜色
    private int selectedOuterColor = 0XFFFFFFFF;// 选中时外圆颜色

    private int errorInnerColor = 0XFFF59C28;// 密码错误时内圆颜色
    private int errorOuterColor = 0XFFF59C28;// 密码错误时外圆颜色
    ...
    /**
     * 0x001 第一次设置,0x002手势解锁
     */
    private int type;
    /**
     * 设置不能滑屏
     */
    private boolean touchEnableLong = true;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (p != null) {
                    isChecking = true;
                    mCompleteListener.onStartSlide();
                    mCompleteListener.onSlide(p.value + "");
                }
            ...
            case MotionEvent.ACTION_MOVE:
                if (isChecking) {
                    ...
                    mCompleteListener.onSlide(getPassword());
                }
                break;
        ...
        if (!isChecking && mCompleteListener != null) {
            ...
            if ((selectedPoints.size() < pwdMinLen || selectedPoints.size() > pwdMaxLen) || (password != null && !getPassword().equals(password))) {
                mCompleteListener.onPwdShortOrLong(selectedPoints.size());
                passwordError();
            } else {
                if (type == GesturePassWordActivity.TYPE_GESTUREPASSWORD_REPEATEDLY) {
                ...
                } else if (type == GesturePassWordActivity.TYPE_GUSTUREPASSWORD_LOGIN) {
                    if (getPassword().equals("userPassword")) {
                        mCompleteListener.onComplete(getPassword());
                        clearPassword(CLEAR_TIME);
                    } else {
    ...
    private void drawLine(Point start, Point end, Canvas canvas, Paint paint) {
        double d = MathUtils.distance(start.x, start.y, end.x, end.y);
        float cosA = (float) ((end.x - 50 - start.x + 50) / d);
        float sinA = (float) ((end.y - 50 - start.y + 50) / d);
...
//				Paint temP = null;
                switch (p.state) {
                    case Point.STATE_CHECK:
                        selectedPaint.setShader(null);
                        selectedPaint.setStrokeWidth(AndroidUtil.dip2px(getContext(), 1));
                        selectedPaint.setColor(selectedOuterColor);
                        selectedPaint.setStyle(Paint.Style.STROKE);
                        canvas.drawCircle(p.x, p.y, outerRadius, selectedPaint);
                        selectedPaint.setColor(selectedInnerColor);
                        selectedPaint.setStyle(Paint.Style.FILL);
                        RadialGradient linearGradient = new RadialGradient(p.x, p.y, innerRadius * 2, 0xAAFFFFFF, 0X00FFFFFF, Shader.TileMode.MIRROR);
                        selectedPaint.setShader(linearGradient);
                        canvas.drawCircle(p.x, p.y, innerRadius * 2, selectedPaint);
                        selectedPaint.setShader(null);
                        canvas.drawCircle(p.x, p.y, innerRadius, selectedPaint);

                        break;

                    case Point.STATE_CHECK_ERROR:
                        isError = true;
                        errorPaint.setColor(0X66F59C28);
                        errorPaint.setStyle(Paint.Style.FILL);
                        canvas.drawCircle(p.x, p.y, outerRadius, errorPaint);
                        errorPaint.setStrokeWidth(AndroidUtil.dip2px(getContext(), 1));
                        errorPaint.setColor(errorOuterColor);
                        errorPaint.setStyle(Paint.Style.STROKE);
                        canvas.drawCircle(p.x, p.y, outerRadius, errorPaint);
                        errorPaint.setColor(errorInnerColor);
                        errorPaint.setStyle(Paint.Style.FILL);
                        canvas.drawCircle(p.x, p.y, innerRadius, errorPaint);
                        break;

                    case Point.STATE_NORMAL:
                        normalPaint.setShader(null);
                        normalPaint.setStrokeWidth(AndroidUtil.dip2px(getContext(), 1));
                        normalPaint.setColor(outerColor);
                        normalPaint.setStyle(Paint.Style.STROKE);
                        canvas.drawCircle(p.x, p.y, outerRadius, normalPaint);
                        break;
                }
...
//					drawArrow(canvas, arrowPaint, tp, p, innerRadius);
    ...
    private void init() {
        ...
        for (int i = 0; i < mPoints.length; i++) {
            float y = 0;
            if (i == 0)
                y = offsetY + spacing+AndroidUtil.dip2px(getContext(),5);
            else
                y = offsetY + (3 * i) * spacing + spacing+AndroidUtil.dip2px(getContext(),5);
            for (int j = 0; j < mPoints[i].length; j++) {
    ...
    private void initPaints() {
        ...
        this.linePaint.setColor(0X44FFFFFF);
        this.linePaint.setStyle(Paint.Style.STROKE);
        this.linePaint.setAntiAlias(true);
        this.linePaint.setStrokeWidth(this.innerRadius * 1.1f);
    ...
    public LockPattern hideLocus() {
        this.arrowColor = android.R.color.transparent;
        this.lineColor = android.R.color.transparent;
    ...
    public void setTouchEnableLong(boolean touchEnableLong) {
        this.touchEnableLong = touchEnableLong;
